functions {
  vector map_rect_likelihood(vector phi,vector params, real[] data_r, int[] data_i) {
    ...
    matrix[K -1, K-1] Omega_alpha = to_matrix(phi[(4*K-3):(4*K- 4 + (K-1)*(K-1))], (K-1), (K-1));
    matrix[K -1, K-1] Omega_beta = to_matrix(phi[(4*K- 3 + (K-1)*(K-1)):(4*K- 4 + 2*(K-1)*(K-1))], (K-1), (K-1));
...
model {
  ...
  // Prepare shared data
    ...
    vector[4*K -4 + 2*(K-1)*(K-1)] phi = append_row(aloc_vec, append_row(bloc_vec, append_row(asd_vec,append_row(bsd_vec, append_row(omega_a_vec, omega_b_vec)))));
